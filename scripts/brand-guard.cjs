...
const HEX_CODES = Object.freeze({
  PRIMARY_MAGENTA: "#CE4B95",
  PRIMARY_TEAL: "#55ABA8",
  GOLD: "#F9E8C3",
  INK: "#0B0D0F",
});
const CANONICAL_DISPLAY = 'linear-gradient(135deg, var(--smh-magenta) 0%, var(--smh-teal) 100%)';
const CANONICAL_GRAD = 'linear-gradient(135deg,var(--smh-magenta)0%,var(--smh-teal)100%)';
...
for(const file of files){
  ...
  if(
    file !== TOKENS_FILE &&
    /background\s*:\s*linear-gradient[^;]*#ce4b95[^;]*#55aba8/i.test(txt)
  ){
  ...
  const surfaceBlocks = txt.match(/\.champagne-surface\s*\{[^}]*\}/g) || [];
