...
type PreviewContent = Awaited<ReturnType<typeof loadTreatmentPreviewContent>>;

type SlotArgs = {
  config: PreviewTreatmentConfig;
...
export type ChampagneTreatmentTemplateProps = TemplateProps & {
  renderHud?: (args: SlotArgs) => ReactNode;
  renderSchema?: (args: SchemaSlotArgs) => ReactNode;
};
    ...
    <section aria-labelledby="how-it-works" className={styles.section}>
      <div className={styles.sectionHeader}>
        ...
        <p className={styles.sectionLead}>
          Tab-ready overview mirroring CB_TABS_OVERVIEW_V1. Replace placeholders with schema-connected steps once the HowTo pack is synced.
        </p>
      </div>
    ...
    <section className={styles.section} aria-labelledby="finance-heading">
      <div className={`${styles.glass} ${styles.financeCard}`}>
        <div className={styles.sectionHeader}>
          ...
          <p className={styles.sectionLead}>
            Placeholder finance band keyed to <strong>{planGroup ?? "pending-plan-group"}</strong>. Swap in live finance modules once plan mappings are approved.
          </p>
        </div>
...
export default async function ChampagneTreatmentTemplate(props: ChampagneTreatmentTemplateProps) {
  const {
    slug,
    schemaKey,
    treatmentName,
  ...
  const hud = renderHud?.({ config, previewContent, schemaSlug, schemaKey, faqKey });
  const schema = renderSchema?.({ config });
